...
3. **データ形式を自動判定**（ラップタイム形式 or 経過時間形式）
4. 各選手のデータを読み取り、適切に変換
   - **経過時間形式**: 経過時間からネットラップタイムを計算
   - **ラップタイム形式**: ラップタイムから経過時間を計算して表示
5. スタートループを検出して、ベストラップ判定から除外
6. 変換されたセルに視覚的なマーカーを表示

**特徴:**

- 元の時間データは `data-original-time` 属性に保存され、ホバー時に表示
- DNS（欠場）や未完走の選手は自動的にスキップ
- ページ構造の変更にも柔軟に対応
- **主催者によって異なるデータ形式に自動対応**
...
- **v1.3.4**: データ形式自動判定機能追加（ラップタイム形式 vs 経過時間形式）
