...
#### データ形式判定 (content.js:228-316) ← UPDATED!
...
**3つ以上のデータがある場合（推奨）**:
1. 1位の選手（最初の有効な行）から最大5つの有効なラップタイムを取得
2. 連続する差分を計算（例: time[1] - time[0], time[2] - time[1], ...）
3. 差分の変動係数（CV = 標準偏差 / 平均）を計算
4. CVが0.2未満 かつ 値が単調増加（最後の値 > 最初の値 × 1.2）→ **経過時間形式**
5. それ以外 → **ラップタイム形式**

**2つしかデータがない場合（フォールバック）**:
1. 2つの有効なラップタイムを取得
2. 比率を計算（2つ目 / 1つ目）
3. 比率が1.3倍以上 → **経過時間形式**
4. 比率が1.3倍未満 → **ラップタイム形式**

**背景**:

一部の主催者レースでは、経過時間ではなく周回ごとのラップタイムが直接掲載されています（例: https://data.cyclocross.jp/race/24051）。また、2周目から計測を開始しているレースでは、最初の比率が通常より小さくなります（例: 1.5倍）。このような多様なケースに対応するため、複数のデータポイントを使った統計的な判定ロジックを実装しました。

**例**:

- **経過時間形式（1周目から計測）**: 6:13.2, 12:28.2, 18:46.8 → 差分: 6:15.0, 6:18.6（CV ≈ 0.04）→ 経過時間形式
- **経過時間形式（2周目から計測）**: 12:20.4, 18:29.4, 24:38.5 → 差分: 6:09.0, 6:09.1（CV ≈ 0.002）→ 経過時間形式
- **ラップタイム形式**: 6:13.2, 6:15.0, 6:18.6 → 差分: 1.8秒, 3.6秒（CV ≈ 0.47）→ ラップタイム形式
